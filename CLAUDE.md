...
The bridge writes to SQLite from two sources: live `events.Message` (`handleMessage`) and WhatsApp's bulk `events.HistorySync` (`handleHistorySync`), which arrives in bursts after pairing. Both funnel into `storeMessageEvent`, and that is load-bearing — the two paths drifted apart once and it cost 151 messages.

### Rules for the message handlers

**History sync messages must go through `client.ParseWebMessage`.** whatsmeow only calls `UnwrapRaw` on live events, so a history message read straight off its `WebMessageInfo` is still wrapped — `deviceSent`, `ephemeral`, `viewOnce`, `documentWithCaption`, and edits. An edit in particular arrives as a bare `protocolMessage` holding the new body under the *original* message's ID; extracting fields directly finds no text and drops it. `ParseWebMessage` unwraps all of that and re-files the edit onto the message it edits.

**Write the chat row before its messages.** `messages` holds a foreign key to `chats(jid)`, so on a fresh store every message inserted before its chat is rejected outright. `storeConversation` does both in order; do not separate them.

**`last_message_time` only moves forward.** `StoreChat` upserts with a comparison rather than `INSERT OR REPLACE`, because history sync and on-demand backfill deliver batches of old messages that would otherwise drag a chat backwards in `list_chats`.

**Say what you skip.** The `content == "" && mediaType == ""` guard is the last stop before a message disappears, and it logs the protobuf field name. A silent drop is indistinguishable from a message that never arrived, and telling those apart afterwards is expensive. Expect `couldn't find sender` warnings on group system entries — those carry no content and were always dropped; the warning is the logging working, not a fault.

### Detecting gaps

whatsmeow writes a row into `whatsmeow_message_secrets` for every message it receives — live *and* from history sync — before the bridge's handler runs. Anti-joining it against `messages.db` separates messages the bridge dropped from messages WhatsApp never sent:

```bash
sqlite3 store-US/messages.db "
attach 'store-US/whatsapp.db' as w;
select (select count(*) from w.whatsmeow_message_secrets) received,
       (select count(*) from messages) stored,
       (select count(*) from w.whatsmeow_message_secrets s
          where not exists (select 1 from messages m where m.id = s.message_id)) dropped;"
```

`dropped` should only grow by reactions and protocol messages, which carry nothing to store. It cannot see server-side omissions — nothing local can, by definition.

**History sync is not a source of truth and never becomes one.** It is generated by the phone, best-effort, and a measured sync omitted ~3% of a month. Never re-link to migrate hosts: copy the whole `store-<name>/` directory, which preserves the device identity so the new host is the same linked device. Never run two copies of one session at once.
...
Copy `messages.db` aside before doing it, always. Re-authenticating replaces a complete record with the phone's best-effort backfill, which can leave you with fewer messages than you started with. The copy is then the only way to fill those gaps back in, by inserting rows absent from the new store — dedupe on message id alone, never on `(id, chat_jid)`, because the `@lid` migration files the same message under different chat JIDs in different stores.
...
| A message visible on the phone is absent | It only ever arrived via history sync, which is lossy. Check `whatsmeow_message_secrets` to tell "bridge dropped it" from "never sent" |
| `FOREIGN KEY constraint failed` on store | A message is being written before its chat row. See the handler rules above |
